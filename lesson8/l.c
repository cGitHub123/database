...
MetaCommandResult doMetaCommand(InputBuffer *input_buffer, Table *table) {
    ...
    if (strcmp(input_buffer->buffer, ".exit") == 0) {
        ...
        return MEAT_COMMAND_SUCCESS;
    } else if (strcmp(input_buffer->buffer, ".constant")) {
        printf("Constsant:\n");
        print_constants();
        return MEAT_COMMAND_SUCCESS;
    } else {
...
void set_node_typo(void *node, NodeType type) {
    uint8_t value = type;
    *((uint8_t *) (node + NODE_TYPE_OFFSET)) = value;
}

void initialize_left_node(void* node) {
    set_node_typo(node, NODE_LEAF);
    *leaf_node_num_cells(node) = 0;
}
